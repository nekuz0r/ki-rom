...
check_synthetic() {
	...
	if [ ! -x fixtures/mkcorpus ] || [ ! -x fixtures/mkromimage ] ||
		[ ! -x fixtures/mkbadstream ]; then
	...
	#
	# All five archives come from fixtures/mkbadstream, which derives the header
	# offsets from KIPACK_PACKED_DATA instead of this script restating them as
	# byte positions. The last two could not be built here in any case: the ROM
	# window is 512 KB and load_rom zero-fills it, so running off the end takes
	# a chain of table offsets, each landing where the next is read from.
	#
	# The zero file count and the non-multiple-of-4 size are the two that hung
	# the decoder indefinitely before the guards went in, so they are the
	# regression tests for it. The other three have always been refusals.
	for bad in magic count size seek walk; do
		fixtures/mkbadstream "$WORK/syn/corpus.packed" "$bad" \
			"$WORK/syn/bad-$bad.packed"
	done

	reject_case "bad magic"                    "$WORK/syn/bad-magic.packed"
	reject_case "zero file count"              "$WORK/syn/bad-count.packed"
	reject_case "size not a multiple of 4"     "$WORK/syn/bad-size.packed"
	reject_case "table seek past ROM window"   "$WORK/syn/bad-seek.packed"
	reject_case "Huffman walk past ROM window" "$WORK/syn/bad-walk.packed"
